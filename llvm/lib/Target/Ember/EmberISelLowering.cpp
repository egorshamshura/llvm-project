...
#include "Ember.h"
#include "EmberMachineFunctionInfo.h"
...
#define DEBUG_TYPE "Ember-lower"

using namespace llvm;

static const MCPhysReg ArgGPR32s[] = {Ember::RH9, Ember::RH10, Ember::RH11, Ember::RH12};
static const MCPhysReg ArgGPR64s[] = {Ember::R9, Ember::R10, Ember::R11, Ember::R12};
 
void EmberTargetLowering::ReplaceNodeResults(SDNode *N, SmallVectorImpl<SDValue> &Results, SelectionDAG &DAG) const {
    llvm_unreachable("");
}
...
#include "EmberGenCallingConv.inc"
...
SDValue EmberTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI, SmallVectorImpl<SDValue> &InVals) const {
    EMBER_DUMP_RED
    ...
    CCInfo.AnalyzeCallOperands(Outs, STI.is64bit() ? CC_Ember64 : CC_Ember32);
    ...
    for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
        ...
        if (!StackPtr.getNode())
            StackPtr = DAG.getCopyFromReg(Chain, DL, (STI.is64bit() ? Ember::RH1 : Ember::R1), PtrVT);
        SDValue Address =
    ...
    Chain = DAG.getNode(EmberISD::CALL, DL, NodeTys, Ops);
    ...
    RetCCInfo.AnalyzeCallResult(Ins, STI.is64bit() ? RetCC_Ember64 : RetCC_Ember32);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
    ...
    SDValue EmberTargetLowering::LowerFormalArguments(
    ...
    SmallVector<CCValAssign, 16> ArgLocs;
    CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
    CCInfo.AnalyzeFormalArguments(Ins, STI.is64bit() ? CC_Ember64 : CC_Ember32);
    ...
    if (IsVarArg) {
        ArrayRef<MCPhysReg> ArgRegs = ArrayRef(STI.is64bit() ? ArgGPR64s : ArgGPR32s);
        unsigned Idx = CCInfo.getFirstUnallocated(ArgRegs);
        const TargetRegisterClass *RC = (STI.is64bit() ? &Ember::GPR64RegClass : &Ember::GPR32RegClass);
        MachineFrameInfo &MFI = MF.getFrameInfo();
        MachineRegisterInfo &RegInfo = MF.getRegInfo();
        EmberFunctionInfo *UFI = MF.getInfo<EmberFunctionInfo>();
...
bool EmberTargetLowering::CanLowerReturn(
    ...
    if (!CCInfo.CheckReturn(Outs, STI.is64bit() ? RetCC_Ember64 : RetCC_Ember32))
        return false;
...
SDValue EmberTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                bool IsVarArg,
    ...
    const EmberSubtarget &STI = MF.getSubtarget<EmberSubtarget>();
    ...
    CCInfo.AnalyzeReturn(Outs, STI.is64bit() ? RetCC_Ember64 : RetCC_Ember32);
...
bool EmberTargetLowering::isLegalAddressingMode(const DataLayout &DL,
